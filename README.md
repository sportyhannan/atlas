...
Twelve public registries, all linked back to one `investigators` table. The move that made everything work was treating identity resolution as *data*. We precomputed the links between registries and stored them with confidence scores, instead of trying to fuzzy match strings at query time.

Three Mastra agents on top: one parses the query, one searches, one scores. The search agent has two tools. One lists candidates. The other pulls a full profile for each by fanning out across every source table at once.

## What broke

Spent an hour on `result.object` being `undefined`. Turns out Mastra's default `maxSteps` is 5, and our agent uses a step per candidate, so we'd run out before it could write a response. Also, if an agent uses tools *and* wants structured output, you have to pass a separate `structuredOutput.model`, or the tool loop eats the final response. Bumping to 20 steps and adding the structuring model fixed it.
...
Semantic search over abstracts with pgvector. Ranking that knows the sponsor. PDF export. More registries. A feedback loop so the rating agent learns from shortlist picks.
